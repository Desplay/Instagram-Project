...
import { Post } from 'src/common/entity/post.entity';
import { File } from 'src/common/entity/file.entity';
import { CloudinaryService } from '../systems/cloudinary/cloudinary.service';

@Injectable()
export class PostsService {
  constructor(@InjectModel('Post') private readonly PostModel: Model<Post>, private readonly cloudService: CloudinaryService) {}
  ...
  async updatePost(id: string, post: any, file: File | null): Promise<Post | undefined> {
    if (file) {
      if (!(await this.cloudService.deleteFile(id))) throw new Error('Delete file failed');
      const url = await this.cloudService.uploadFile(file, id);
      post.imageUrl = url;
    }
    return await this.PostModel.findByIdAndUpdate(id, post, { new: true }).exec();
  }
