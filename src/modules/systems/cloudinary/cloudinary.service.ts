...
export class CloudinaryService {
  ...
  async uploadFile(file: File, postID: string): Promise<string> {
    ...
    const result = await new Promise((resolve, reject) => {
      const upload = v2.uploader.upload_stream({ folder: 'nestjs_graphql_mongodb_basic' }, (error, result) => {
  ...
  async deleteFile(postID: string): Promise<boolean> {
    try {
      const response = await v2.api.resources({ type: 'upload', prefix: 'nestjs_graphql_mongodb_basic' });
      const file = response.resources.find((file: any) => file.public_id === postID);
